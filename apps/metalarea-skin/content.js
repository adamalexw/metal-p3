(async () => {
  ...
  const getFlagCode = (country) => {
    const codes = {
      'sweden': 'se', 'finland': 'fi', 'norway': 'no', 'germany': 'de',
      'usa': 'us', 'united states': 'us', 'uk': 'gb', 'united kingdom': 'gb',
      'russia': 'ru', 'france': 'fr', 'poland': 'pl', 'italy': 'it',
      'canada': 'ca', 'brazil': 'br', 'australia': 'au', 'netherlands': 'nl',
      'japan': 'jp', 'greece': 'gr', 'switzerland': 'ch', 'austria': 'at',
      'denmark': 'dk', 'spain': 'es', 'mexico': 'mx', 'chile': 'cl',
      'argentina': 'ar', 'colombia': 'co', 'portugal': 'pt', 'belgium': 'be',
      'ukraine': 'ua', 'czech republic': 'cz', 'romania': 'ro', 'hungary': 'hu',
      'turkey': 'tr', 'ireland': 'ie', 'iceland': 'is', 'serbia': 'rs',
      'croatia': 'hr', 'bulgaria': 'bg', 'slovakia': 'sk', 'slovenia': 'si',
      'estonia': 'ee', 'latvia': 'lv', 'lithuania': 'lt', 'belarus': 'by',
      'new zealand': 'nz', 'south africa': 'za', 'china': 'cn', 'south korea': 'kr',
      'indonesia': 'id', 'malaysia': 'my', 'philippines': 'ph', 'thailand': 'th',
      'india': 'in', 'peru': 'pe', 'venezuela': 've', 'ecuador': 'ec', 'bolivia': 'bo',
      'uruguay': 'uy', 'paraguay': 'py', 'costa rica': 'cr', 'panama': 'pa',
      'puerto rico': 'pr', 'cuba': 'cu', 'dominican republic': 'do'
    };
  ...
  if (isForum) {
    ...
    if (ipbWrapper) {
      ...
      // Extract topics
      topicLinks = Array.from(ipbWrapper.querySelectorAll('a[href*="showtopic="]'))
        .filter(a => !a.href.includes('view=getlastpost') && !a.href.includes('view=getnewpost') && !a.href.includes('pid='));
      ...
      if (pageCurrent && pageCurrent.parentElement) {
        ...
        const stLinks = Array.from(document.querySelectorAll('a[href*="st="]'))
          .filter(a => !a.href.includes('getlastpost') && a.textContent.match(/^\d+$/));
        if (stLinks.length > 0) {
  ...
  async function fetchTopicDetails(topic) {
      try {
        ...
        if (cachedData) {
          ...
          clone.querySelectorAll('p, div, tr, li').forEach(el => { el.prepend('\n'); el.append('\n'); });
          ...
          let tracklistBlockMatch = postText.match(/(?:Tracklist|Треклист)[\s\S]*?(?:Total playing|Line-up|Download|Скачать|Technical|Hidden|\*|$)/i);
        ...
        if (firstPost) {
          ...
          try {
             localStorage.setItem(cacheKey, JSON.stringify({
                timestamp: Date.now(),
                coverUrl, country, fetchedGenre, tracksCount, trackNames, totalDuration, downloadLinks
             }));
          } catch(e) {}
        }
