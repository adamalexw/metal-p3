...
export const selectTracksRequired = createSelector(selectTracks, selectTracksLoading, (tracks, loading) => (loading ? false : !tracks?.length));
...
export const selectTracksRequiredById = (id: number) => createSelector(selectTracksById(id), selectAlbumById(id), (tracks, album) => (album?.tracksLoading ? false : !tracks?.length));
