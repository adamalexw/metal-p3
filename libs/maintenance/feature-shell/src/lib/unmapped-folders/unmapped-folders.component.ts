...
import { addNewAlbum, loadAlbums } from '@metal-p3/shared/data-access';
...
export class UnmappedFoldersShellComponent {
  ...
  onSearch(folder: string) {
    this.store.dispatch(loadAlbums({ request: { skip: 0, take: 40, criteria: folder } }));
  }

  onAdd(folder: string) {
    this.store.dispatch(addNewAlbum({ folder }));
  }
