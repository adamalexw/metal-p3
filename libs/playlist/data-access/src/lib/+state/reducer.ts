...
export const playlistFeature = {
  ...
  reducer: createReducer(
    ...
    on(PlaylistActions.createSuccess, (state, { playlist }): PlaylistState => adapter.addOne(playlist, { ...state, active: playlist.id })),
    on(PlaylistActions.loadPlaylistSuccess, (state, { id }): PlaylistState => ({ ...state, active: id })),
    on(PlaylistActions.saveSuccess, (state, { playlist }): PlaylistState => adapter.updateOne({ id: playlist.id, changes: { ...playlist } }, state)),
    on(PlaylistActions.deleteSuccess, (state, { id }): PlaylistState => adapter.removeOne(id, { ...state, active: undefined })),
    ...
    on(PlaylistActions.transferError, (state): PlaylistState => ({ ...state, transferring: false })),
